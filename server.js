...
app.get('/api/courses/:id', (req, res) => {
  db.get('SELECT * FROM courses WHERE id = ?', [req.params.id], (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
    row ? res.json(row) : res.status(404).json({ message: 'Course not found' });
  });
});
...
app.put('/api/courses/:id', (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
      this.changes ? res.json({ message: 'Course updated' }) : res.status(404).json({ message: 'Course not found' });
    }
  );
});
...
app.delete('/api/courses/:id', (req, res) => {
  db.run('DELETE FROM courses WHERE id = ?', [req.params.id], function (err) {
    if (err) return res.status(500).json({ error: err.message });
    this.changes ? res.json({ message: 'Course deleted' }) : res.status(404).json({ message: 'Course not found' });
  });
});
